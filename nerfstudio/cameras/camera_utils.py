...
def mei_unproject(coords: torch.Tensor, params: torch.Tensor) -> torch.Tensor:
    """Unproject 2D pixel coordinates to 3D ray directions using the MEI omnidirectional model.

    Implements the inverse of the Mei & Rives (ICRA 2007) unified projection model:
    pixel -> normalized plane -> undistort -> lift from mirror sphere.

    Args:
        coords: (N, 2) pixel coordinates, where N = 3*K (base, x-offset, y-offset).
        params: (K, 10) per-ray camera parameters [fx, fy, cx, cy, xi, k1, k2, p1, p2, _pad].

    Returns:
        (N, 3) unit ray directions in camera frame (OpenGL convention).
    """
    K = params.shape[0]
    params_expanded = params.repeat(3, 1)  # (3*K, 10)

    fx = params_expanded[:, 0]
    fy = params_expanded[:, 1]
    cx = params_expanded[:, 2]
    cy = params_expanded[:, 3]
    xi = params_expanded[:, 4]
    k1 = params_expanded[:, 5]
    k2 = params_expanded[:, 6]
    p1 = params_expanded[:, 7]
    p2 = params_expanded[:, 8]

    # Step 1: pixel -> normalized (distorted) coordinates on the projection plane
    mx_d = (coords[:, 0] - cx) / fx
    my_d = (coords[:, 1] - cy) / fy

    # Step 2: iteratively invert the radial + tangential distortion (k1, k2, p1, p2)
    # Reuse the existing solver by packing as [k1, k2, 0, 0, p1, p2]
    dist_6 = torch.stack([k1, k2, torch.zeros_like(k1), torch.zeros_like(k1), p1, p2], dim=-1)
    m_distorted = torch.stack([mx_d, my_d], dim=-1)
    m_undistorted = radial_and_tangential_undistort(m_distorted, dist_6)
    mx = m_undistorted[..., 0]
    my = m_undistorted[..., 1]

    # Step 3: MEI lifting — invert the mirror projection  m = P_s.xy / (P_s.z + xi)
    # to recover the 3D point on the unit sphere.
    r2 = mx * mx + my * my
    disc = torch.clamp(1.0 + (1.0 - xi * xi) * r2, min=0.0)
    lam = (xi + torch.sqrt(disc)) / (1.0 + r2)
